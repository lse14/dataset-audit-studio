...
这是一个独立开发的训练集筛选工具，参考 [Krea 2 Technical Report](https://www.krea.ai/blog/krea-2-technical-report) 公开的数据治理思路：在保留审美和概念多样性的前提下，优先处理重复或过度代表的样本、无法可靠描述的样本、明显偏差或伪影，以及当前目标分辨率难以稳定建模的复杂样本。报告还明确将质量和审美分数用于剔除极低质量样本，而不是按分数过采样。

本项目将这些原则落地为本地数据集的可追溯筛选、人工复核和安全导出流程；它不复刻 Krea 2 的训练系统、数据集或模型，也不隶属、受赞助于或获得 Krea 官方背书。
...
当前公开代码没有已登记但尚未完成的核心功能项；以下是后续规划，而不是已经交付的能力或 Krea 官方路线图：

- 将项目内运行时、安装和原生路径选择能力扩展到 Windows 11 以外的平台。
- 为不同训练目标、分辨率和素材来源提供更系统的筛选策略校准与可复现基准报告。
- 增加 caption、标签和其他元数据的质量复核建议，同时保持默认不直接改写源标注的安全边界。
- 扩展可独立验证的模型适配器和模型比较流程；新模型仍只生成审核候选，不自动删除或排除素材。

Krea 2 技术报告描述的是面向基础模型训练的大规模数据和训练基础设施；本项目聚焦单个本地训练集的筛选与导出，不以复现该规模为目标。
