...
from dataset_audit_studio.app.modular_clustering import (
    MODULAR_CLUSTERING_COMPONENT_IDS,
...
class ClusteringComponentPlan:
    component_id: str
    model_ids: tuple[str, ...]


@dataclass(frozen=True)
class ModularClusteringRunSummary:
    task_id: str
...
ComponentAssetWaiter = Callable[[WorkerToken, ClusteringComponentPlan], RuntimeAssets | None]


def build_clustering_component_plan(
    task_config: dict[str, Any],
    *,
    registry: ComponentRegistry = COMPONENT_REGISTRY,
) -> tuple[ClusteringComponentPlan, ...]:
    return tuple(
        ClusteringComponentPlan(
            resolved.definition.manifest.id,
            resolved.definition.model_ids(resolved.config),
        )
        for resolved in registry.resolve_task_config(task_config)
        if resolved.definition.manifest.id in MODULAR_CLUSTERING_COMPONENT_IDS
    )


class ModularClusteringCoordinator:
    def __init__(
        self,
        database: Database,
        tasks: TaskService,
        *,
        ...
        registry: ComponentRegistry = COMPONENT_REGISTRY,
        process_runner: ProcessRunner = run_modular_clustering_component_subprocess,
    ...
    def run(self, token: WorkerToken) -> ModularClusteringRunSummary:
        task = self.tasks.get_task(token.task_id)
        plan = build_clustering_component_plan(task.config, registry=self.registry)
        ...
        for item in plan:
            ...
            if current.status != TaskStatus.SEMANTIC_CLUSTERING.value:
                return self._summary(token.task_id, summaries)
        finalize_modular_clustering(self.tasks, token, component_order=component_order)
        return self._summary(token.task_id, summaries)

    def run_component(
        self,
        token: WorkerToken,
        item: ClusteringComponentPlan,
        *,
        ...
        if summary.get("component_id") != item.component_id:
            raise RuntimeError("Clustering subprocess returned the wrong component")
        current = self.tasks.get_task(token.task_id)
        if (
            current.status == TaskStatus.SEMANTIC_CLUSTERING.value
            and summary.get("component_complete") is not True
        ):
            raise RuntimeError(f"Clustering component {item.component_id} is incomplete")
        return summary
    ...
    def _wait_for_assets(
        ...
        if self.model_service is None:
            if item.model_ids:
                raise RuntimeError(
                    f"Clustering component {item.component_id} requires the model service"
                )
    ...
    def _summary(self, task_id: str, summaries: list[dict[str, Any]]):
        return ModularClusteringRunSummary(
            task_id,
            tuple(summaries),
            self.tasks.get_task(task_id).status,
        )
